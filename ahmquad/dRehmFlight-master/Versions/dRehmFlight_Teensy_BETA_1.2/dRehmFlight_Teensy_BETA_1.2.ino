...
#include <SR04.h>     //reading ultrasonic sensor data 
#include <SoftwareSerial.h> //Bluetooth LE communication
...
float Kp_roll_angle = 0.14;    //Roll P-gain - angle mode
float Ki_roll_angle = 0.3;    //Roll I-gain - angle mode
float Kd_roll_angle = 0.035;   //Roll D-gain - angle mode (if using controlANGLE2(), set to 0.0)
float B_loop_roll = 0.9;      //Roll damping term for controlANGLE2(), lower is more damping (must be between 0 to 1)
float Kp_pitch_angle = 0.14;   //Pitch P-gain - angle mode
float Ki_pitch_angle = 0.3;   //Pitch I-gain - angle mode
float Kd_pitch_angle = 0.035;  //Pitch D-gain - angle mode (if using controlANGLE2(), set to 0.0)
...
//Default values for controller parameters
//float i_limit = 25.0;     //Integrator saturation level, mostly for safety (default 25.0)
//float maxRoll = 30.0;     //Max roll angle in degrees for angle mode (maximum 60 degrees), deg/sec for rate mode
//float maxPitch = 30.0;    //Max pitch angle in degrees for angle mode (maximum 60 degrees), deg/sec for rate mode
//float maxYaw = 160.0;     //Max yaw rate in deg/sec
...
// Declaring the trigger and echo pins for each of the three ultrasonic sensors
const int S1Trigger = 24;
const int S1Echo = 26;
const int S2Trigger = 5;
const int S2Echo = 4;
const int S3Trigger = 2;
const int S3Echo = 3;

//Creating sensor objects for the 3 ultrasonic sensors
SR04 S1 = SR04(S1Echo, S1Trigger);
SR04 S2 = SR04(S2Echo, S2Trigger);
SR04 S3 = SR04(S3Echo, S3Trigger);

//HM-10 BLE Module RX and TX pins
const int TX = 1;
const int RX = 0;
...
//Ultrasonic sensor measurements in bytes since BTSerial only writes in bytes
byte rawS1;
byte rawS2;
byte rawS3;

//Ble bytes to send to HM-10 and c and NL for text formatting
char c = ' ';
boolean NL = true;
byte bleS1;
byte bleS2;
...
void setup() {
  ...
  BTserial.begin(9600); //BLE serial
  ...
  bleS1 = map(S1.Distance(), 2, 799, 0, 255);
  bleS2 = map(S2.Distance(), 2, 799, 0, 255);
  bleS3 = map(S3.Distance(), 2, 799, 0, 255);
  bleGyroX = GyroX;
  bleGyroY = GyroY;
  bleGyroZ = GyroZ;

  bleGyroX = map(GyroX, -250, 250, 0, 255);
  bleGyroY = map(GyroY, -250, 250, 0, 255);
  bleGyroZ = map(GyroZ, -250, 250, 0, 255);
...
void loop() {
  ...
  //printSonarData();      //Prints distance measured (expected: 2cm to 800cm)
  //printMappedSonarData();//Prints mapped distance values to fit in the byte range of 0-255 (expected: 0cm to 255cm)
  //printAvgSonarData();   //Prints average distance values averaged per 4 measurments (expected 2cm to 800cm)
  //printBLEWrite();       //Prints what's being written to HM-10
  //printLoopRate();      //prints the time between loops in microseconds (expected: microseconds between loop iterations)



  //enableBLEComm(); //Allow HM-10 and Serial Monitor to communicate
  //writeBLEData();  //Write data to send from the HM-10 to phone
...
void controlMixer() {
  ...
  */
  ...
  //    s1_command_scaled = 0.5 * thro_des;
  //    s2_command_scaled = 0.5 * thro_des;
...
void printSonarData() {
  if (current_time - print_counter > 10000) {
    ...
    Serial.print(F(" S1: "));
    Serial.print(S1.Distance());
    Serial.print(F(" S2: "));
    Serial.print(S2.Distance());
    Serial.print(F(" S3: "));
    Serial.println(S3.Distance());
  }
}

void printAvgSonarData() {
  if (current_time - print_counter > 10000) {
    print_counter = micros();
    Serial.print(F(" S1: "));
    Serial.print(S1.DistanceAvg());
    Serial.print(F(" S2: "));
    Serial.print(S2.DistanceAvg());
    Serial.print(F(" S3: "));
    Serial.println(S3.DistanceAvg());
  }
}

void printMappedSonarData() {
  if (current_time - print_counter > 10000) {
    print_counter = micros();
    Serial.print(F("Mapped S1:   "));
    Serial.print(map(S1.Distance(), 2, 799, 0, 255));
    Serial.print(F("   Mapped S2:   "));
    Serial.print(map(S2.Distance(), 2, 799, 0, 255));
    Serial.print(F("   Mapped S3:   "));
